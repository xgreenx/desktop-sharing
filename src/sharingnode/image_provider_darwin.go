...
func NewImageProvider(options *ScreenOptions, displaysInfo *DisplaysInfo, resolution int) (*ImageProvider, error) {
	...
	err = provider.optionsScreen.Set("list_devices", "true")
	if err != nil {
		goto Error
	}
	...
	provider.avInputFormat = avformat.FindInputByShortName("avfoundation")
	if provider.avInputFormat == nil {
		err = errors.New("pAVInputFormat is nil")
		goto Error
	}

	err = provider.avFormatContext.OpenInput("1", provider.avInputFormat, provider.optionsScreen)
	if err != nil {
		goto Error
	}
