...
func NewImageProvider(options *ScreenOptions, displaysInfo *DisplaysInfo, resolution int) (*ImageProvider, error) {
	...
	provider.avInputFormat = avformat.FindInputByShortName("gdigrab")
	...
	err = provider.optionsScreen.Set("offset_x", fmt.Sprintf("%d", offsetX))
	if err != nil {
		goto Error
	}

	err = provider.avFormatContext.OpenInput(fmt.Sprintf("desktop"), provider.avInputFormat, provider.optionsScreen)
	if err != nil {
		goto Error
	}
